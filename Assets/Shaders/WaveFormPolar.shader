Shader "Custom/WaveForm Polar"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 to_polar(const float2 cartesian)
            {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                i.uv -= 0.5;
                i.uv *= 2;
                float2 uv = to_polar(i.uv);
                uv.x = frac(uv.x);

                float xx = uv.y - frac(_Time.y * 1);
                if (xx < 0)
                {
                    xx += 1;
                }
                uv.y = xx;
                uv.x = saturate(frac(uv.x + _Time.y * 0.5));
